...
export function ApiManager() {
  const [keys, setKeys] = useState<ApiKey[]>([
    { id: '1', nome: 'Modal Kimi K3', provedor: 'modal', chave: 'wk-sf2hP1ueSZI0MN3L2vztJp.ws-HycRo9SIrXndyK147Ppuff', ativo: true, usoHoje: 47, limiteDiario: 1000 },
  ...
  return (
    <div className="h-screen flex flex-col" style={{ background: 'var(--bg-primary)' }}>
      ...
      <div className="p-6 flex items-center justify-between" style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface)' }}>
        ...
        <button onClick={() => setShowModal(true)} className="btn-primary flex items-center gap-2" style={{ background: 'var(--accent)' }}>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {keys.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p style={{ color: 'var(--text-tertiary)' }}>Adicione uma chave para começar a usar IA nos seus fluxos</p>
          </div>
        ) : (
          <div className="space-y-4">
            {keys.map((key) => {
              ...
              return (
                <div key={key.id} className="card p-5 transition-all">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2 flex items-center gap-2">
                        ...
                        <button onClick={() => toggleShowKey(key.id)} className="p-1 transition-colors" style={{ color: 'var(--text-tertiary)' }}>
                          {showKey ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                        </button>
                        <button onClick={() => handleCopy(key.chave, key.id)} className="p-1 transition-colors" style={{ color: 'var(--text-tertiary)' }}>
                          {copiedId === key.id ? <Check className="w-4 h-4" style={{ color: 'var(--sage)' }} /> : <Copy className="w-4 h-4" />}
                        </button>
                      </div>
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <button onClick={() => setKeys(prev => prev.map(k => k.id === key.id ? {...k, ativo: !k.ativo} : k))} className="p-2 rounded-lg transition-colors" style={{ color: 'var(--text-tertiary)' }} title={key.ativo ? 'Desativar' : 'Ativar'}>
                        {key.ativo ? <Zap className="w-5 h-5" style={{ color: 'var(--sage)' }} /> : <Zap className="w-5 h-5" />}
                      </button>
                      <button onClick={() => handleEdit(key)} className="p-2 rounded-lg transition-colors" style={{ color: 'var(--text-tertiary)' }} title="Editar">
                        <Edit3 className="w-5 h-5" />
                      </button>
                      <button onClick={() => handleDelete(key.id)} className="p-2 rounded-lg transition-colors" style={{ color: 'var(--text-tertiary)' }} title="Excluir">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.5)' }}>
          <div className="card w-full max-w-lg">
            <div className="p-4 flex items-center justify-between" style={{ borderBottom: '1px solid var(--border)' }}>
              ...
              <button onClick={closeModal} className="p-2 rounded-lg transition-colors" style={{ color: 'var(--text-tertiary)' }}>✕</button>
            </div>
            <div className="p-4 space-y-4">
              <div>
                <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text-secondary)' }}>Nome</label>
                <input type="text" value={form.nome} onChange={(e) => setForm({...form, nome: e.target.value})} className="input-field" placeholder="Ex: OpenAI Produção" />
              </div>
              <div>
              ...
              {provedores[form.provedor].url && (
                <a href={provedores[form.provedor].url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm" style={{ color: 'var(--terracotta)' }}>
            ...
            <div className="p-4 flex justify-end gap-3" style={{ borderTop: '1px solid var(--border)' }}>
              ...
              <button onClick={handleSave} disabled={!form.nome || !form.chave} className="btn-primary flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed" style={{ background: 'var(--accent)' }}>
