...
const actionConfig: Record<HITLActionType, { label: string; icon: any; cor: string; bg: string; alerta: string }> = {
  payment: { label: 'Pagamento', icon: CreditCard, cor: 'var(--rose-400)', bg: 'rgba(244, 63, 94, 0.08)', alerta: 'Transacao financeira detectada' },
  ...
  sensitive_data: { label: 'Dados Sensiveis', icon: Lock, cor: 'var(--violet-400)', bg: 'rgba(139, 92, 246, 0.08)', alerta: 'Dados pessoais ou confidenciais' },
          ...
          <div className="card w-full max-w-md" style={{ 
            ...
            <div className="p-4 space-y-3">
              <div className="p-3 rounded-lg" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)' }}>
                <p className="text-[10px] font-medium mb-1" style={{ color: 'var(--text-muted)' }}>Acao</p>
                <p className="text-xs font-semibold" style={{ color: 'var(--text-primary)' }}>{selectedAction.titulo}</p>
              </div>

              <div className="p-3 rounded-lg" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)' }}>
                <p className="text-[10px] font-medium mb-1" style={{ color: 'var(--text-muted)' }}>Descricao</p>
                <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>{selectedAction.descricao}</p>
              </div>

              {selectedAction.dados && (
                <div className="p-3 rounded-lg max-h-32 overflow-y-auto" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)' }}>
                  <p className="text-[10px] font-medium mb-1" style={{ color: 'var(--text-muted)' }}>Dados</p>
                  <pre className="text-[10px] font-mono" style={{ color: 'var(--text-tertiary)' }}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)' }}>
          <div className="card w-full max-w-lg max-h-[80vh] flex flex-col">
            <div className="p-3 flex items-center justify-between" style={{ borderBottom: '1px solid var(--border)' }}>
              ...
              <button onClick={() => setShowLog(false)} className="text-xs" style={{ color: 'var(--text-muted)' }}>Fechar</button>
            </div>
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
              {hitlLog.length === 0 ? (
                <p className="text-center text-xs py-8" style={{ color: 'var(--text-muted)' }}>Nenhum registro</p>
              ) : (
                hitlLog.map((action) => {
                  ...
                  return (
                    <div key={action.id} className="flex items-center gap-3 p-2.5 rounded-lg" style={{ background: 'var(--surface)' }}>
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium truncate" style={{ color: 'var(--text-primary)' }}>{action.titulo}</p>
                        <p className="text-[10px]" style={{ color: 'var(--text-muted)' }}>{action.timestamp.toLocaleString('pt-BR')}</p>
                      </div>
                      <span className="text-[10px] px-1.5 py-0.5 rounded" style={{
                        background: action.status === 'approved' ? 'rgba(52, 211, 153, 0.1)' : 'rgba(244, 63, 94, 0.1)',
                        color: action.status === 'approved' ? 'var(--emerald-400)' : 'var(--rose-400)'
                      }}>
